const conditionallyAddPlural = (noun, cnt) => {
    if (cnt > 1) {
      return `${noun}s`;
    }
    return noun;
...
export const timeAgo = (dateString) => {
  ...
  export function localizeAndPrettify(dateString) {
    const date = new Date(dateString);
    return date.toLocaleString();
  }
  
  // Assuming conditionallyAddPlural is defined somewhere else in your code
